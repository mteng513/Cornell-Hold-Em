...
module Game_Engine = struct
  ...
  let remove_elt (e: 'a) (l: 'a list) : 'a list =
  ...
  (* Computes the max of 4 numbers. *)
    let four_max (a: int) (b: int) (c: int) (d: int) : int = 
      max (max a b) (max c d)
  ...
  let score_calculation (hand: card list): int =
    ...
    (* [score g_state] takes in the global_state [g_state]
  ...
  let rec signal_bet (g_state : global_state) : unit = 
    print_endline (""); 
  ...
  let award_chips (g_state: global_state) : unit = 
  ...
  let remove_players (g_state: global_state) : unit = 
    (* update chips *)
  ...
  let end_message (g_state: global_state) : unit = 
